...
class ElasticSearchInteraction:
    ...
    def index_team_content(self, index_name, doc_type, team_data):
        ...
        while team_index < 190:
            temp = {}
            temp_float = re.sub('<[^<]+?>', '', team_data[team_index-1]).strip()
            temp['team_name'] = temp_float
            temp_float = re.sub('<[^<]+?>', '', team_data[team_index + 1])
            temp['total_shot_ratio'] = float(temp_float)
            temp_float = re.sub('<[^<]+?>', '', team_data[team_index + 2])
            ...
            print "added " + team_data[team_index-1]
            team_index += 9
