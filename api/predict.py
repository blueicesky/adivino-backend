...
#
# es = Elasticsearch(['http://54.245.215.12:9200'], connection_class=MyConnection)
...
class predict:
    ...
    def __init__(self):
        ...
        self.es = Elasticsearch(['http://54.245.215.12:9200'], connection_class=MyConnection)
        ...
        res = self.es.search(index=es_index_name, body={"size": 100, "query": {"match_all": {}}})
        res = res['hits']['hits']
    ...
    def process_new_feature(self, json_doc):
        ...
        # for item in columns:
        #     print(item)        #
        # columns = list(self.predictors.columns)
        self.alg.fit(self.df[self.predictors], list(self.df['score']))
        importance = self.alg.feature_importances_
        dict_importance = {}
        ...
        for item in self.predictors:
            dict_importance[item] = importance[i]
            i += 1

        for item in dict_importance:
            print item, dict_importance[item]
